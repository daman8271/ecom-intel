...
matrix("Discount Analysis", lambda c: pct_fraction(round(statistics.mean([x['discount_pct'] for x in c if x['discount_pct'] is not None]), 1)) if any(x['discount_pct'] is not None for x in c) else None, '0.0%', scale=True, scale_rev=True)
