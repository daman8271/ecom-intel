...
| **Blinkit** | **Windows + Mac Pro residential sessions, auth-required** | Production starts the Windows team shard at 07:00 IST and uses `/Users/danny./VPS-Migration/scripts/run_blinkit_mac_to_vps.sh` under LaunchAgent `com.danny.blinkit-mac-to-vps` at 07:15 IST for the Mac shard. It loads `/Users/danny./VPS-Migration/secrets/blinkit-auth-state.json`, exports `BLINKIT_REQUIRE_AUTH=1`, `BLINKIT_OOS_PROBE=1`, `BLINKIT_PDP_OOS_PROBE=1`, and `BLINKIT_PDP_PRICE_PROBE=1`, and VPS ingest defaults to `BLINKIT_REQUIRE_AUTH_DROP=1`. If the Mac is unreachable, `tools/cron/blinkit_vps_kvm_fallback.sh` runs authenticated VPS+KVM1 shards using `/opt/ecom-intel/secrets/blinkit-auth-state.json` and still promotes only through the same ingest gates. Do not run or accept anonymous/unprobed Blinkit drops; `summary.auth_session`, `summary.auth_required`, `summary.auth_verified`, `summary.auth_verified_pincodes == summary.pincodes_total`, `summary.oos_probe_enabled`, `summary.pdp_oos_probe_enabled`, `summary.pdp_price_probe_enabled`, and `summary.pdp_price_probe_failed == 0` must be present/truthy or clean as applicable. Accepted drops direct-send the main workbook to the Ecom WhatsApp group through `tools/whatsapp/send_blinkit_main_direct.sh` and the not-listed workbook to the configured direct contact through `tools/whatsapp/send_blinkit_not_listed_direct.sh`; both are gated by `tools/cron/blinkit_quality_monitor.sh`. |
| **Other platforms** (Flipkart/Flipkart-Minutes) | **DIRECT (no proxy)** — leave `PROXY_*` as-is per-platform | They already work from the datacenter IP. Don't burn proxy GB on them. Only Zepto is wired to the proxy today; wire others the same way **only if** they start getting blocked. |
