...
- **Selection (owner upgraded 2026-07-12: 0 → FULL):** per city, take **100% of universe pins** — every India Post pin in all 24 cities (1,550 total). The urban-first ranking is retained only as list ORDER (tracked pins, then urban, then rural), so proven pins scrape first; nothing is excluded. Pins with no usable lat/lon in the CSV get the city's mean coordinate as fallback so no platform drops them.
