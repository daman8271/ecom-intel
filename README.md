...
- **Opt-in, non-breaking:** turned on per run via `COVERAGE_FULL=1` (or an explicit
  `PINCODES_FILE`). With the flag unset, runs are **byte-for-byte unchanged** (anchor
  `pincodes.json`). The anchor config is never touched — it's the rollback.
- **Full configs:** `platforms/<p>/pincodes.full25.json` (one entry per pincode,
  generated by `tools/pincodes/gen_full_configs.py`).
