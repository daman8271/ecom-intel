...
# PHASE 2 SPLIT (owner-approved 2026-07-07; revised 2026-07-09): flipkart scrapes
# OFF-BOX on KVM1 at store-open hours (~07:30 IST; tools/cron/kvm1_trio_launch.sh
# fires the box, bin/kvm1_run_trio.sh there dead-drops results back through
# tools/cron/kvm1_ingest.sh -> run.sh SCRAPE_RESULT_DROP -> the same review gates +
# batch spool). Zepto is Mac Pro primary via tools/cron/zepto_macpro_guard.sh after
# KVM1 showed repeat 429 pressure. flipkart-minutes STAYS in this chain (its logged-in Flipkart API is
...
PLATFORMS="${PLATFORMS_OVERRIDE:-flipkart-minutes amazon amazon-fresh amazon-now}"  # kvm1: flipkart · macpro: blinkit bigbasket swiggy zepto (ingest.sh feeders)
