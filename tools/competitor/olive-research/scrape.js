...
// olive-research: env-gate the resolve retry budget so speculative rural (non-serviceable)
// pincodes FAIL FAST instead of burning 4x6 polls (~38s) each. Serviceable pins usually
// resolve on attempt 0, so a lower budget keeps accuracy while collapsing the dead-pin cost.
const RESOLVE_ATTEMPTS = parseInt(process.env.RESOLVE_ATTEMPTS || '4', 10);
const RESOLVE_POLLS = parseInt(process.env.RESOLVE_POLLS || '6', 10);
...
async function scrapeOne(browser, rec) {
  ...
  try {
    ...
    let activeLoc = {};
    for (let attempt = 0; attempt < RESOLVE_ATTEMPTS && !resolved; attempt++) {
      ...
      for (let poll = 0; poll < RESOLVE_POLLS && !resolved; poll++) {
...
if (require.main === module) (async () => {
  ...
  if (COMPETITOR_MODE) {
    ...
    // goal #54 patch (ISOLATED COPY ONLY): also persist a lean per-pincode serviceability
    // record so the availability denominator can distinguish serviceable-but-no-olive and
    // non-serviceable pincodes from those that simply had olive rows. rows live in allRows.
    const perPinLean = perPin.map((p) => ({
      city: p.city, pincode: p.pincode, state: p.state, target_state: p.target_state,
      lat: p.lat, lon: p.lon, known_serviceable: p.known_serviceable,
      store_id: p.store_id || '', store_name: p.store_name || '',
      resolved: !!p.resolved, blocked: p.blocked || null, n_rows: (p.rows || []).length,
    }));
    fs.writeFileSync(outJson, JSON.stringify({ summary, perPin: perPinLean, allRows }, null, 2));
