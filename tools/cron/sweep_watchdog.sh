...
#   The noon batch is armed by ONE once-a-day cron tick:
#     30 0 * * *  ... deadline_sweep.sh 12:00
...
#   once-a-day job is gone for the whole day — and nothing noticed until the 10:00
#   mailer timed out 3h later and shipped 2 of ~10 reports. Single trigger, no
...
# guard the noon production slot only
[ "$SLOT" = "12:00" ] || { say "non-noon slot '$SLOT' — ignoring"; exit 0; }
...
REMAIN=$(( T - NOW ))
# predicted serial runtime; fail-safe floor of 7h (observed ~6.3-7h) so a lowball
# prediction can never fool us into thinking there's plenty of time.
...
launch(){ # start the real sweep, detached, same env as the primary cron line
  ...
  setsid env LEAD_MAX="${LEAD_MAX:-39600}" COVERAGE_DAILY="${COVERAGE_DAILY:-1}" \
...
if [ "$REMAIN" -ge "$NEED" ]; then
  launch
  notify "⚠️ Jivo noon sweep watchdog
The 00:30 cron did NOT arm today's ($DATE) noon sweep. Watchdog launched it as catch-up at $(date -d "@$NOW" '+%H:%M') IST — on track to finish by 12:00. Likely a crontab-edit race dropped the launch tick."
else
  launch
  notify "🚨 Jivo noon batch AT RISK
Today's ($DATE) noon sweep was never armed and it is now $(date -d "@$NOW" '+%H:%M') IST — too little runtime to finish a full chain by 12:00. Launched best-effort; the 12:00 batch will likely be LATE or PARTIAL. Manual check needed."
fi
exit 0
