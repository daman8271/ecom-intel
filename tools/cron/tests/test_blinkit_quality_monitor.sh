...
BAD_PRICE_PROBE_FAILED_OUT="$(mktemp)"
...
trap 'rm -f "$GOOD" "$BAD" "$BAD_UNVERIFIED" "$BAD_PRICE_PROBE_DISABLED" "$BAD_PRICE_PROBE_FAILED" "$BAD_STOCK_UNVERIFIED" "$BAD_CANOLA_OOS" "$BAD_CANOLA_PRICE" "$BAD_CANOLA_UNVERIFIED_PRICE" "$GOOD_WORKBOOK" "$GOOD_NOT_LISTED_WORKBOOK" "$BAD_MAIN_MISSING_NL" "$BAD_NOT_LISTED_WORKBOOK" "$BAD_WORKBOOK" "$GOOD_OUT" "$GOOD_WORKBOOK_OUT" "$MISSING_NOT_LISTED_OUT" "$BAD_MAIN_MISSING_NL_OUT" "$BAD_NOT_LISTED_WORKBOOK_OUT" "$BAD_OUT" "$BAD_UNVERIFIED_OUT" "$BAD_PRICE_PROBE_DISABLED_OUT" "$BAD_PRICE_PROBE_FAILED_OUT" "$BAD_STOCK_UNVERIFIED_OUT" "$BAD_CANOLA_OOS_OUT" "$BAD_CANOLA_PRICE_OUT" "$BAD_CANOLA_UNVERIFIED_PRICE_OUT" "$BAD_WORKBOOK_OUT" "$ROOT/logs/blinkit_quality_monitor-2099-01-01.log" "$ROOT/logs/blinkit_quality_monitor-2099-01-01.state"' EXIT

python3 - "$FIXTURE" "$GOOD" "$BAD" "$BAD_UNVERIFIED" "$BAD_PRICE_PROBE_DISABLED" "$BAD_PRICE_PROBE_FAILED" "$BAD_STOCK_UNVERIFIED" "$BAD_CANOLA_OOS" "$BAD_CANOLA_PRICE" "$BAD_CANOLA_UNVERIFIED_PRICE" <<'PY'
import json, sys
src, good_path, bad_path, bad_unverified_path, bad_price_probe_disabled_path, bad_price_probe_failed_path, bad_stock_unverified_path, bad_canola_oos_path, bad_canola_price_path, bad_canola_unverified_price_path = sys.argv[1:]
...
good["summary"]["pdp_price_probe_attempted"] = 1
good["summary"]["pdp_price_probe_checked"] = 1
good["summary"]["pdp_price_probe_failed"] = 0
...
bad_price_probe_disabled["summary"].pop("pdp_price_probe_attempted", None)
bad_price_probe_disabled["summary"].pop("pdp_price_probe_checked", None)
bad_price_probe_disabled["summary"].pop("pdp_price_probe_failed", None)
...
bad_price_probe_failed = json.loads(json.dumps(good))
bad_price_probe_failed["summary"]["pdp_price_probe_attempted"] = 2
bad_price_probe_failed["summary"]["pdp_price_probe_checked"] = 1
bad_price_probe_failed["summary"]["pdp_price_probe_failed"] = 1
json.dump(bad_price_probe_failed, open(bad_price_probe_failed_path, "w", encoding="utf-8"))
...
BLINKIT_MONITOR_DRYRUN=1 \
BLINKIT_MONITOR_DATE=2099-01-01 \
BLINKIT_MONITOR_RESULT="$BAD_PRICE_PROBE_FAILED" \
BLINKIT_MONITOR_REPORT=/tmp/no-such-blinkit-report.xlsx \
  "$MONITOR" test > "$BAD_PRICE_PROBE_FAILED_OUT"
grep -q '"ok": false' "$BAD_PRICE_PROBE_FAILED_OUT"
grep -q 'pdp_price_probe_failed' "$BAD_PRICE_PROBE_FAILED_OUT"
