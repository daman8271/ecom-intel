...
#   SWEEP_DOWNSTREAM==1 && SLOT==10:00 && no PLATFORMS_OVERRIDE && no RUNNER_OVERRIDE
...
check "normal 10:00 sweep, rc=0 -> fires competitor"       fire 10:00 0
check "sweep failed (rc=1) -> does NOT fire"                no   10:00 1
check "old noon slot (12:00) now does NOT fire"            no   12:00 0
check "wrong slot (15:00) -> does NOT fire"                 no   15:00 0
check "test-mode RUNNER_OVERRIDE set -> does NOT fire"      no   10:00 0 RUNNER_OVERRIDE=/bin/true
check "test-mode PLATFORMS_OVERRIDE set -> does NOT fire"   no   10:00 0 PLATFORMS_OVERRIDE=zepto
check "kill switch SWEEP_DOWNSTREAM=0 -> does NOT fire"     no   10:00 0 SWEEP_DOWNSTREAM=0

# prove ORDER: price-scraper advance is recorded BEFORE competitor
: > "$FIRED"; fire_downstream 10:00 0 >/dev/null 2>&1
order_ok=no
