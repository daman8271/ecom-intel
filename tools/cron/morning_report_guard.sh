...
# BigBasket NATIONAL + BigBasket PINCODE + BLINKIT + ZEPTO must reach the morning
...
# late; (c) no Mac Blinkit scrape -> the device workflow missed delivery. These
# failures were not represented in the old partial-batch success signal.
#
...
#
#   early  (~08:40 IST, before the 10:30 batch delivery): make sure BB national is
...
#   check  (~10:35 IST for the 10:30 batch, plus a quiet 11:05 Blinkit deadline
#          pass): verify all required reports routed today, send a completeness
...
ZEP_RPT="output/Jivo-Zepto-Live-Report-${TODAY}.xlsx"
...
zepto_sent(){
  local receipt="logs/delivery-receipts/${TODAY}/$(basename "$ZEP_RPT").json" sha
  [ -f "$ZEP_RPT" ] && [ -f "$receipt" ] || return 1
  sha="$(sha256sum "$ZEP_RPT" | awk '{print $1}')" || return 1
...
try:
    ...
    raise SystemExit(1)
raise SystemExit(0 if data.get("sha256") == sys.argv[2] and data.get("messageId") else 1)
PY
}
batch_posted(){        # did the 10:00 mailer actually post the batch to the Ecom group?
  ...
  # complete-set receipt line AFTER today's mailer-start marker in mailer.log.
  awk -v d="$TODAY" '$0 ~ ("mailer start slot=.* date=" d) {f=1}
       f && /WhatsApp: posted complete delivery set to Ecom team group/ {found=1}
...
if [ "$PASS" = "early" ]; then
  if ! pincode_routed && [ -f "$PIN_RPT" ]; then
    LOG "BB pincode workbook is ready and queued for the 10:30 Ecom batch"
  fi
...
NAT_OK=0; PIN_OK=0; BLK_OK=0; BLK_DUE=0; ZEP_OK=0; BATCH_OK=0
...
if [ "$PIN_OK" != "1" ]; then
  if [ -f "$PIN_RPT" ]; then
    LOG "BB pincode workbook exists but sent marker is absent -> retrying idempotent batch delivery"
    if [ "$DRY" = "1" ]; then
      PIN_NOTE="[dryrun] would retry the idempotent batch delivery for BB pincode."
    elif MAILER_SKIP_EMAIL=1 MAILER_SKIP_WAIT=1 ./tools/mailer/mail_price_data.sh am && pincode_routed; then
      PIN_OK=1
...
$(emoji $PIN_OK) BigBasket pincode-wise (10:30 Ecom batch)
$(blinkit_emoji) Blinkit (Ecom group; ${BLINKIT_DEADLINE} IST deadline)
$(emoji $ZEP_OK) Zepto (10:30 Ecom batch)
...
# Blinkit uses only the Mac Pro + Windows device team for production scraping.
if [ "$BLK_OK" != "1" ] && [ "$BLK_DUE" = "1" ]; then
  SUMMARY="$SUMMARY
...
if [ "$NAT_OK" = "1" ] && [ "$PIN_OK" = "1" ] && [ "$BLK_OK" = "1" ] && [ "$ZEP_OK" = "1" ] && [ "$BATCH_OK" = "1" ]; then
  if [ "$QUIET_OK" = "1" ]; then
    LOG "all required reports routed — OK (quiet)"
  else
    LOG "all required reports routed — OK"
    alert "✅ All required morning reports routed for ${TODAY} (BB national, BB pincode, Blinkit, Zepto, price-data batch)."
  fi
elif [ "$NAT_OK" = "1" ] && [ "$PIN_OK" = "1" ] && [ "$ZEP_OK" = "1" ] && [ "$BATCH_OK" = "1" ] && [ "$BLK_DUE" != "1" ]; then
